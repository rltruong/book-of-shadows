...
function drawGradientCircle(canvas, moods, size) {
  // Back the canvas at device resolution (3x on iPhone) so the circle's
  // antialiased edge is computed at native density instead of being drawn
  // at CSS pixels and upscaled, which blurs the rim.
  const dpr =
...
function MoodPie({ counts }) {
  ...
  const r = 130;
  const labelR = r * 0.65;
  const FS = 14; // SVG renders 1:1, so this is real pixels
  const paths = [];
  ...
  entries.forEach((e, i) => {
    ...
    if (e.count === total) {
      ...
      paths.push(
        <text key={'tn' + i} x={cx} y={cy - 8.5} textAnchor="middle" dominantBaseline="middle" fontSize={FS} fontWeight="600" fill={labelFill}>
          {e.label}
        </text>,
        <text key={'tp' + i} x={cx} y={cy + 8.5} textAnchor="middle" dominantBaseline="middle" fontSize={FS} fill={labelFill}>
          {pctLabel(e.count)}
        </text>
      );
      return;
    ...
    const fits = Math.hypot(halfW, 15) <= bound;

    if (fits) {
      const lx = cx + labelR * Math.cos(mid);
      const ly = cy + labelR * Math.sin(mid);
      paths.push(
        <text key={'tn' + i} x={lx} y={ly - 8.5} textAnchor="middle" dominantBaseline="middle" fontSize={FS} fontWeight="600" fill={labelFill}>
          {e.label}
        </text>,
        <text key={'tp' + i} x={lx} y={ly + 8.5} textAnchor="middle" dominantBaseline="middle" fontSize={FS} fill={labelFill}>
          {pctText}
        </text>
      );
    } else {
  ...
  [-1, 1].forEach((side) => {
    ...
    group.forEach((o, gi) => {
      if (gi > 0) o.y = Math.max(o.y, group[gi - 1].y + 18);
    });
  });
...
function svgTextWidth(str, weight, px) {
  ...
  _measureCtx.font = `${weight} ${px}px Inter, ui-sans-serif, system-ui, -apple-system, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif`;
